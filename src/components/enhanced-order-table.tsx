...
import { MlGetOrdersResponse, MLGetOrdersResults, Result } from "@/types/ml/orders"
                ...
                {order.order_items.map((item, index) => (
                  <div key={index} className="mb-2">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Quantity: {item.quantity} x {formatCurrency(item.unit_price, order.currency_id)}
                    </div>
                  </div>
                ))}
              </TableCell>
                    ...
                    {selectedOrder && (
                      <div className="grid gap-4 py-4">
                        <div>
                          ...
                          <p>Date: {formatDate(selectedOrder.date_created)}</p>
                          <p>Status: {selectedOrder.status}</p>
                          <p>Buyer: {selectedOrder.buyer.nickname}</p>
                          <p>Total: {formatCurrency(selectedOrder.total_amount, selectedOrder.currency_id)}</p>
                        </div>
                        <div>
                          ...
                          {selectedOrder.order_items.map((item, index) => (
                            <div key={index} className="mb-2">
                              ...
                              <p>Price: {formatCurrency(item.unit_price, selectedOrder.currency_id)}</p>
                            </div>
                          ))}
                        </div>
                      </div>
