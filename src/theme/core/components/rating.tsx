...
const MuiRating: Components<Theme>['MuiRating'] = {
  ...
  styleOverrides: {
    iconEmpty: ({ theme }) => ({
      color: varAlpha((theme.vars || theme).palette.grey['500Channel'], 0.48)
    }),
    root: { [`&.${ratingClasses.disabled}`]: { opacity: 0.48 } },
    sizeLarge: { [`& .${svgIconClasses.root}`]: { height: 28, width: 28 } },
    sizeMedium: { [`& .${svgIconClasses.root}`]: { height: 24, width: 24 } },
    sizeSmall: { [`& .${svgIconClasses.root}`]: { height: 20, width: 20 } }
  }
}
